...
local DOApKM to 120. //Desired orbit altitude in kilometers(apoapsis in fact but its circular so Pe~=Ap)
local OINCL to 45. //Desired orbit inclination in degrees
local LiftOffAngle to 86. //Tune-in for more/less brutal gravity turn depending on your rocket
...
local function evalTHRAP { //Used to cut the throttle when apoapsis gets closer to desired apoapsis
	parameter x.
	return (max(0.1, min(1, -CONSTANT:e^((x/10000)-DOApKM/10)+1))).
}

local function g{ //Simply calculates g, takes account of altitude
	return body:mu/(body:radius+altitude)^2.
}
local function rho{ //Air density
	return 1.221*CONSTANT:e^(altitude/-5000).
}

local function tVel{ //Terminal velocity, needs adjustments 
	return sqrt(1250*g()/rho()).
}
...
local function AscentBurn{ //There's probably a more efficient way to do it, by for example coding that in the actual ascent function
	...
	until apoapsis >= DOApKM*1000 - epsilon {
		set STR to heading(90-OINCL, min(LiftOffAngle-10, 90-vAng(up:vector, srfprograde:vector))).
		set THR to evalTHRAP(apoapsis).

		// if altitude<70001 {set THR to evalTHRAP(apoapsis)*evalTHRAPftVel(altitude).}
		// else{set THR to evalTHRAP(apoapsis).}
		UI("Ascent in progress", "", "Terminal velocity : ", tVel()).
		wait 0.01.
	}
	set THR to 0.
}
