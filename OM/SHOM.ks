...
local DOApKM to 82. //Desired orbit altitude in kilometers(apoapsis in fact but its circular so Pe~=Ap)
local OINCL to 0. //Desired orbit inclination in degrees
local LiftOffAngle to 96. //Tune-in for more/less brutal gravity turn depending on your rocket

//Not to set
local THR to 1.
local STR to HEADING(270-OINCL, LiftOffAngle).
...
local function AscentBurn{ //There's probably a more efficient way to do it, by for example coding that in the actual ascent function
	...
	until apoapsis >= DOApKM*1000 - epsilon {
		set STR to heading(270-OINCL, max(LiftOffAngle, 90+(75/(DOApKM*1000))*altitude)).
		// set STR to heading(270-OINCL, max(LiftOffAngle, 90+vAng(up:vector, srfprograde:vector))).
...
local function Ascent { //Gravity turn ascent function
	...
	until altitude >= body:atm:height { //keeps the apoapsis at desired altitude if the burn ended while still in atmosphere
		...
		set STR to heading(270-OINCL, 90+vAng(up:forevector, prograde:vector)).
